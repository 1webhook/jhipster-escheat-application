...
import com.sme.myapp.config.KafkaSseConsumer;
import com.sme.myapp.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class HhipsterescheatclaimKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/hhipsterescheatclaim-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/hhipsterescheatclaim-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/hhipsterescheatclaim-kafka/unregister"));
                return;
            }
        }
